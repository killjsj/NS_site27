...
namespace NS_site27_heavy.heavy.Module.dage
{
    ...
    ///
    /// <para>
    /// The MicroHID does not use the firearm hitreg at all, so <see cref="ExactRayPatch"/> never
    ...
    /// spread, so only the redirect half of the ability applies. Second, the direction is read off
    /// <c>PlayerCameraReference.forward</c> <em>after</em> FpcBacktracker has overwritten the camera
    /// rotation with the client's claimed one, so it must be redirected from inside that scope.
    /// </para>
    ///
    /// <para>
    ...
    /// <c>ServerRequestBacktrack</c>, so PrimaryFire, ChargeFire and BrokenFire are all covered —
    ...
    {
        /// <summary>
        /// Acquisition cone. Wider than the firearm value because MicroHID range is 4-10 m, so a
        ...
        // BacktrackAll is synchronous, server-main-thread, and one item at a time, so a single
        ...
        // either way so a single failure does not disable the ability for the rest of the round.
        ...
        private static void InvokeWithRedirect()
        {
            ...
            if (owner == null || !(owner.roleManager.CurrentRole is PlayerRoles.FirstPersonControl.IFpcRole))
            {
                inner();
                return;
            }
            ...
            try
            {
                ...
                // FirearmDamageHandler does), so a headshot is worth nothing here — prefer the
                ...
                if (found)
                {
                    ...
                    TryLookAt(owner,target.CenterOfMass);
